...
module.exports = function(secret) {
  ...
  // Cookie is coming back in format: "eat=fhjkdfshafshjkfasdhkjfas"
  function parseCookie(cookieText) {
    if (cookieText) { return cookieText.split('=')[cookieText.length-1]; }
  }

  return function eatAuth(req, res, next) {
    var eatoken = req.headers.eat || req.body.eat || req.params.eat;

    console.log("EATOKEN IS: ", eatoken);
    if (!eatoken) {
      console.log('no token found');
      return res.status(401).json({error: true, msg: 'please sign in to do that'});
    }
    // TODO: ADD TOKEN RESET ON CLIENT SIDE WHEN BODY.RESET === true
